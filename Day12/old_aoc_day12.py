...
def perm_string(input_string, damaged_data, count=0):
    ...
    if input_string.find("?") > -1:
        ...
        if compare_lists(temp_damaged_data, damaged_data):
            ...
            count = perm_string(
                trim_dots("." + input_string[len(next_section) :]),
                subtract_list(damaged_data, temp_damaged_data),
                count,
            )
        ...
        if input_string.count("#") >= sum(damaged_data):
            ...
            count = perm_string(
                merge_strings(input_string, next_section[:-1] + "#"),
                damaged_data,
                count,
            )
    #            MEMO[(input_string,str(damaged_data))] = count
...
def total_perms(spring_string, damaged_springs):
    ...
    possible_damaged_springs = produce_combinations(
        unknown_spring_locations, sum(damaged_springs) - spring_string.count("#")
    )
    for possible in possible_damaged_springs:
...
def permute_string(spring_string, n, damaged_springs, perm_count=0):
    ...
    if (
        n == len(spring_string)
        and calculate_number_record(spring_string) == damaged_springs
    ):
    ...
    possible_springs = min(
        sum(damaged_springs) - spring_string.count("#"), next_section.count("?")
    )
    ...
    for possible in combinations:
        ...
        if compare_lists(new_data, damaged_springs):
            perm_count = permute_string(
                merge_strings(spring_string, new_string), n, damaged_springs, perm_count
            )
    return perm_count
...
def extract_next_section(spring_string, n):
    ...
    for i in range(n, len(spring_string)):
        ...
        if char == "?":
            ...
            if found_hash:
                print(
                    f"     also found_hash = {found_hash} and returning '{output_string}'"
                )
                return output_string
